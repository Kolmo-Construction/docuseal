...
Rails.application.configure do
  ...
  # Allow embedding the signing page inside another origin (e.g. kolmo.io)
  # when FRAME_ANCESTORS env var is set. Falls back to Rails' default
  # SAMEORIGIN when unset.
  if ENV['FRAME_ANCESTORS'].present?
    ancestors = ENV.fetch('FRAME_ANCESTORS').split(/[\s,]+/).reject(&:empty?)
    config.action_dispatch.default_headers = config.action_dispatch.default_headers.except('X-Frame-Options')
    config.content_security_policy do |policy|
      policy.frame_ancestors :self, *ancestors
    end
  end
